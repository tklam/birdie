...
class Skiplist(SkiplistAbstractBase, collections.MutableMapping):
    ...
    def get_exact_and_just_smaller(self, key):
        """Returns a tuple of: \
                - item with given index if it is found; \
                - item with the index that is just smaller than the given one"""
        node, prevs = self._scan(key)
        if node is None:
            return None, self[prevs[0].key]
        return node.data, self[prevs[0].key]
